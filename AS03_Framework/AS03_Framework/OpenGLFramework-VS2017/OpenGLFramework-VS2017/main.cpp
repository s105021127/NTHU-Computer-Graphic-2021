...
bool Mag_Nearest_Or_Linear = true; // initial is nearest
bool Min_Nearest_Or_Linear_mipmap_linear = true; // initial is nearest
...
typedef struct _Offset { // [HW3] // [texture transform] /*--------這要用來做甚麼阿---------*/
	GLfloat x;
	GLfloat y;
...
typedef struct // [HW3]
{
	...
	GLuint isEye; // [texture transform]  /*--------這該如何抓阿(目前用檔名抓)-----------*/
	vector<Offset> offsets; /*------這要用來做甚麼阿-------*/
...
struct model // [HW3]
{
	...
	bool hasEye; // [texture transform] /*-------------------這該如何抓阿----------------------*/
...
// uniforms location  // [HW3]
GLuint iLocP;
GLuint iLocV;
GLuint iLocM;
GLuint iLocTex0; // texture
GLuint iLocViewPos;
GLuint iLocVerOrPix; // per vertex or per pixel location
GLuint iLocChangeLight;

struct texTransPointer 
{
	GLuint iLocIsEye;
	GLuint iLocXoffset;
	GLuint iLocYoffset;
	...
	GLuint iLocHasEye;
	GLuint iLocCur_eye_offset_idx;
};
texTransPointer texTransPointers;
...
// Render function for display rendering // [HW3] 與之前有些不同
void RenderScene(int per_vertex_or_per_pixel) {	
	...
	// pass texture0 to fragemnt shader [HW3]
	glUniform1i(iLocTex0, 0);

	// pass texture transform to vertex shader [HW3]
	glUniform1i(texTransPointers.iLocHasEye, models[cur_idx].hasEye);
	glUniform1i(texTransPointers.iLocCur_eye_offset_idx, models[cur_idx].cur_eye_offset_idx);
	...
	for (int i = 0; i < models[cur_idx].shapes.size(); i++) 
	{
		...
		// set Ka, Kd, Ks (要記得放到這個for迴圈裡，才會根據不同 shape 對應到不同的 material)
		light.Ka[0] = models[cur_idx].shapes[i].material.Ka[0];
		light.Ka[1] = models[cur_idx].shapes[i].material.Ka[1];
		light.Ka[2] = models[cur_idx].shapes[i].material.Ka[2];
		light.Kd[0] = models[cur_idx].shapes[i].material.Kd[0];
		light.Kd[1] = models[cur_idx].shapes[i].material.Kd[1];
		light.Kd[2] = models[cur_idx].shapes[i].material.Kd[2];
		light.Ks[0] = models[cur_idx].shapes[i].material.Ks[0];
		light.Ks[1] = models[cur_idx].shapes[i].material.Ks[1];
		light.Ks[2] = models[cur_idx].shapes[i].material.Ks[2];

		// 上面儲存完再用 glUniform3fv 傳過去，不然會錯掉
		...
		if (Min_Nearest_Or_Linear_mipmap_linear == true) {
		...
		glUniform1i(texTransPointers.iLocIsEye, models[cur_idx].shapes[i].material.isEye); /////////[不確定]
		//glUniform1i(texTransPointers.iLocXoffset, models[cur_idx].shapes[i].material.offsets[0].x);
...
void KeyCallback(GLFWwindow* window, int key, int scancode, int action, int mods)
{
	if (action == GLFW_PRESS) {
		...
		case GLFW_KEY_J: // switch to shininess editing mode [HW2]
			...
			break;
		case GLFW_KEY_G: // switch the magnification texture filtering mode between nearest / linear sampling
			if (Mag_Nearest_Or_Linear == true) {
				Mag_Nearest_Or_Linear = false;
			}
			else
			{
				Mag_Nearest_Or_Linear = true;
			}
			break;
		case GLFW_KEY_B: // switch the minification texture filtering mode between nearest / linear_mipmap_linear sampling
			if (Min_Nearest_Or_Linear_mipmap_linear == true) {
				Min_Nearest_Or_Linear_mipmap_linear = false;
			}
			else
			{
				Min_Nearest_Or_Linear_mipmap_linear = true;
			}
			break;
		case GLFW_KEY_RIGHT: // Apply eye change on normal order (1-7) 
			models[cur_idx].cur_eye_offset_idx = (models[cur_idx].cur_eye_offset_idx + 1) % (models[cur_idx].max_eye_offset);
			break;
		case GLFW_KEY_LEFT: // Apply eye change on reverse order (7-1)
...
GLuint LoadTextureImage(string image_path) // [HW3]
{
	int channel, width, height;
	int require_channel = 4; // 要求把原先只有RGB 3個channel的格式，轉成RGBA的4個channel了
	stbi_set_flip_vertically_on_load(true); // 能夠在圖像加載時幫助我們翻轉y軸
	...
	if (data != NULL)
	{
		...
		glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, width, height, 0, GL_RGBA, GL_UNSIGNED_BYTE, data); // 產生bug的地方! (因為required_channel=4，所以要用GL_RGBA而非GL_RGB)
	...
	for (int m = 0; m < materials.size(); m++)
	{
		...
		for (int v = 0; v < material_id.size(); v++) 
		{
			...
			if (material_id[v] == m)
			{
				...
				// 如果在.obj文件中?有看到"vc"行，那是因為頂點顏色被省略了
				// 如果沒有指定頂點顏色，則頂點顏色默認為(1,1,1)，即白色
...
void LoadTexturedModels(string model_path) // [HW3]
{
	...
	for (int i = 0; i < materials.size(); i++)
	{
		...
		cout << base_dir + string(materials[i].diffuse_texname) << endl;

		// Check isEye or Not
		string str = "Eye";
		string::size_type idx = string(materials[i].diffuse_texname).find(str);
		if (idx != string::npos)
		{
			cout << "Same " << str << endl;
			material.isEye = 1; //////////////////////////////[暫時]
...
void setUniformVariables() // [HW3]
{
	...
	iLocVerOrPix = glGetUniformLocation(program, "per_vertex_or_per_pixel");
	iLocChangeLight = glGetUniformLocation(program, "change_light");

	// link light attribute
	lightPointers.iLocDir_dir = glGetUniformLocation(program, "directionL.direction");
	lightPointers.iLocDir_pos = glGetUniformLocation(program, "directionL.position");

	lightPointers.iLocPoi_pos = glGetUniformLocation(program, "pointL.position");
	lightPointers.iLocPoi_const = glGetUniformLocation(program, "pointL.constant");
	lightPointers.iLocPoi_linear = glGetUniformLocation(program, "pointL.linear");
	lightPointers.iLocPoi_quad = glGetUniformLocation(program, "pointL.quad");

	lightPointers.iLocSpot_dir = glGetUniformLocation(program, "spotL.direction");
	lightPointers.iLocSpot_pos = glGetUniformLocation(program, "spotL.position");
	lightPointers.iLocSpot_exp = glGetUniformLocation(program, "spotL.exponent");
	lightPointers.iLocSpot_cutoff = glGetUniformLocation(program, "spotL.cutoff");
	lightPointers.iLocSpot_const = glGetUniformLocation(program, "spotL.constant");
	lightPointers.iLocSpot_linear = glGetUniformLocation(program, "spotL.linear");
	lightPointers.iLocSpot_quad = glGetUniformLocation(program, "spotL.quad");

	lightPointers.iLocIa = glGetUniformLocation(program, "light.Ia");
	lightPointers.iLocIp = glGetUniformLocation(program, "light.Ip");
	lightPointers.iLocKa = glGetUniformLocation(program, "light.Ka");
	lightPointers.iLocKd = glGetUniformLocation(program, "light.Kd");
	lightPointers.iLocKs = glGetUniformLocation(program, "light.Ks");
	lightPointers.iLocShininess = glGetUniformLocation(program, "light.shininess");

	// [TODO] Get uniform location of texture
	iLocTex0 = glGetUniformLocation(program, "texture0");

	// texture transform
	texTransPointers.iLocHasEye = glGetUniformLocation(program, "hasEye");
	texTransPointers.iLocIsEye = glGetUniformLocation(program, "isEye");
	texTransPointers.iLocXoffset = glGetUniformLocation(program, "xoffset");
	texTransPointers.iLocYoffset = glGetUniformLocation(program, "yoffset");
	texTransPointers.iLocCur_eye_offset_idx = glGetUniformLocation(program, "cur_eye_offset_idx");
}
...
int main(int argc, char **argv)
{
    ...
    // create window
	GLFWwindow* window = glfwCreateWindow(WINDOW_WIDTH, WINDOW_HEIGHT, "109065504 HW3", NULL, NULL);
    if (window == NULL)
    {
